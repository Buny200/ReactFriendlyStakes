...
const AllBetsPage = ({ language }) => {
  ...
  return (
    <div>
      <h2 className="title">
        {language === "es" ? "Todas las apuestas activas" : "All active bets"}
      </h2>
            ...
            <div className="bet-info">
              <div className="bet-title">
                <span className="title-prefix">
                  {language === "es" ? "Título de la apuesta: " : "Bet Title: "}
                </span>
                {bet.title}
              </div>
              <div className="bet-details">
                ...
                <p>
                  {language === "es" ? "Cantidad de la apuesta" : "Bet Amount"}:{" "}
                  {bet.betAmount}
                </p>
                <p>
                  {language === "es"
                ...
                <p>
                  {language === "es" ? "Estado" : "Status"}: {bet.status}
                </p>
              </div>
            </div>
          </li>
      ...
      {showPopup && selectedBet && (
        <div className="popup" onClick={handlePopupClose}>
          <div className="popup-inner" onClick={(e) => e.stopPropagation()}>
            <h2>
              {language === "es" ? "Detalles de la Apuesta" : "Bet Details"}
            </h2>
            <p>
              {language === "es" ? "Título" : "Title"}: {selectedBet.title}
            </p>
            <p>
              {language === "es" ? "Creador" : "Creator"}:{" "}
              {selectedBet.creator.nickname}
            </p>
            <p>
              {language === "es" ? "Fecha de inicio" : "Start Date"}:{" "}
            ...
            <p>
              {language === "es"
                ? "Número de participantes"
                : "Participants Number"}
              : {selectedBet.participantsNumber}
            </p>
            <p>
              {language === "es" ? "Estado" : "Status"}: {selectedBet.status}
            </p>
          </div>
        </div>
      )}
