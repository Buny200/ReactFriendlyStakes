...
const BetHistoryPopup = ({ betHistory }) => {
    ...
    const lastBetIndex = Math.min(firstBetIndex + betsPerPage, betHistory.betList.length);
    const currentBets = betHistory.betList.slice(firstBetIndex, lastBetIndex);

    return (
        <div className="bet-history-popup">
            <h3 className="popup-title">Historial de Apuestas de {betHistory.nickname}</h3>
